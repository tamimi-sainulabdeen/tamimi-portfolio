...
export function Contact() {
  ...
  const socialLinks = [
    { icon: SiGmail, href: "https://email.com", label: "Email" },
        ...
        <div className="grid lg:grid-cols-2 gap-6 items-start">
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <motion.button
                type="submit"
                className="w-full h-14 rounded-xl bg-gradient-to-r from-primary to-secondary text-primary-foreground flex items-center justify-center gap-2 shadow-lg font-semibold hover:shadow-xl transition-shadow"
                whileHover={{
                  scale: 1.02,
            ...
            <div className="space-y-4">
              {contactInfo.map((info, index) => (
                <motion.a
                  key={info.label}
                  href={info.href}
                  className="flex items-center gap-4 p-4 rounded-xl bg-background border border-border hover:border-primary transition-all group shadow-sm"
                  initial={{ opacity: 0, x: 50 }}
                  whileInView={{ opacity: 1, x: 0 }}
                  viewport={{ once: true }}
